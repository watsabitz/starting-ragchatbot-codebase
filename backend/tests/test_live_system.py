...
class TestLiveVectorStore(unittest.TestCase):
    ...
    def test_existing_courses_data(self):
        ...
        try:
            ...
            self.assertGreater(len(course_titles), 0, "No courses found in vector store")
    ...
    def test_vector_search_functionality(self):
        ...
        try:
            ...
            if len(results.documents) == 0:
                print("WARNING: No search results found - this might indicate indexing issues")
            else:
...
class TestLiveAIGenerator(unittest.TestCase):
    ...
    def test_api_key_validation(self):
        ...
        if not self.ai_generator.client:
            print("WARNING: AI Generator client not initialized - likely invalid API key")
...
class TestLiveRAGSystem(unittest.TestCase):
    ...
    def test_session_functionality(self):
        ...
        try:
            ...
            # Follow-up query
            response2, _ = self.rag_system.query("What did I just say?", session_id="test_session")
...
class TestSystemDiagnostics(unittest.TestCase):
    ...
    def test_environment_configuration(self):
        ...
        print(f"API key format valid: {config.ANTHROPIC_API_KEY.startswith('sk-ant-') if config.ANTHROPIC_API_KEY else False}")
        print(f"Embedding model: {config.EMBEDDING_MODEL}")
        print(f"Max results: {config.MAX_RESULTS}")
        
        # Basic assertions
        self.assertTrue(os.path.exists(config.CHROMA_PATH), "ChromaDB path does not exist")
