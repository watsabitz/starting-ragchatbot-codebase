...
class TestRAGSystem(unittest.TestCase):
    ...
    def test_rag_system_initialization(self):
        ...
        self.mock_session_manager_class.assert_called_once_with(self.config.MAX_HISTORY)
    ...
    def test_query_with_session(self):
        ...
        self.rag_system.tool_manager.get_last_sources = Mock(
            return_value=[{"text": "Course A - Lesson 1", "link": "http://example.com"}]
        )
        self.rag_system.tool_manager.reset_sources = Mock()

        # Execute query with session
        response, sources = self.rag_system.query(
            "Continue discussion", session_id="session_123"
        )
    ...
    def test_query_with_sources_from_search(self):
        ...
        mock_sources = [
            {
                "text": "Machine Learning Course - Lesson 2",
                "link": "http://ml-course.com/lesson2",
            },
    ...
    def test_query_tool_manager_interaction(self):
        ...
        self.rag_system.tool_manager.get_tool_definitions = Mock(
...
class TestRAGSystemIntegrationScenarios(unittest.TestCase):
    ...
    def test_content_query_failure_scenario(self):
        ...
        mock_results = SearchResults(
            documents=[], metadata=[], distances=[], error="Database connection timeout"
        )
        self.mock_vector_store.search.return_value = mock_results

        # AI should still generate response even if search fails
        self.mock_ai_generator.generate_response.return_value = (
            "I encountered an issue searching the course materials."
        )
