...
def main():
    ...
    all_good &= check_file_contains(
        script_path,
        'if (source.link) {',
        "script.js: Added clickable link logic"
    )
    
    all_good &= check_file_contains(
        script_path,
        'target="_blank"',
        "script.js: Links open in new tab"
    )
    
    all_good &= check_file_contains(
        script_path,
        'class="source-link"',
        "script.js: Added CSS class for styling"
    )
    ...
    all_good &= check_file_contains(
        css_path,
        '.source-link {',
        "style.css: Added source link styling"
    )
    ...
    if all_good:
        ...
        print("1. Backend: search_tools.py now retrieves lesson links from vector store")
